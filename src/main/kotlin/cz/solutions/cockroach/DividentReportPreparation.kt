...
object DividentReportPreparation {
    ...
    fun generateDividendReport(
        ...
        // The current PDF layout has one CZ-source/CZK section and foreign-source sections keyed by
        // non-CZK currency. Reject the two cross-combinations rather than silently dropping a row or
        // treating a foreign amount as if it were already CZK.
        czDividends.firstOrNull { it.currency != Currency.CZK }?.let {
            error("Czech-source dividend (country=CZ) paid in ${it.currency.name} is not supported by the current report layout " +
                    "(broker=${it.broker}, symbol=${it.symbol}, date=${DATE_FORMATTER.print(it.date)}). " +
                    "Extend DividentReportPreparation with a converted Czech-source section before reporting it.")
        }
