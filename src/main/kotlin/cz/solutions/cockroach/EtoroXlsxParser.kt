...
object EtoroXlsxParser {
    ...
    private fun parseSheet(xml: String, strings: List<String>, fileName: String): EtoroParseResult {
        ...
        for (rowMatch in rows.drop(1)) {
            ...
            // The sheet has an explicit WHT cell even when the value is zero. Preserve that fact as
            // a zero TaxRecord so downstream validation can distinguish genuine 0 % withholding
            // from a parser that accidentally lost the tax row.
            val taxAmount = if (wht == 0.0) 0.0 else -wht
            taxes.add(TaxRecord(date, taxAmount, Currency.USD, symbol = instrument, broker = BROKER_NAME))
        }
