...
object DegiroAccountStatementParser {
    ...
    fun parse(workbook: Workbook): DegiroParseResult {
        ...
        // Degiro omits the withholding row entirely when the applicable rate is 0 %. The report
        // preparation layer deliberately requires an explicit tax row for every dividend, so add
        // one only when this statement contained no matching tax entry.
        val taxedKeys = taxes.map { Triple(it.date, it.symbol, it.currency) }.toSet()
        dividends.distinctBy { Triple(it.date, it.symbol, it.currency) }
            .filter { Triple(it.date, it.symbol, it.currency) !in taxedKeys }
            .forEach {
