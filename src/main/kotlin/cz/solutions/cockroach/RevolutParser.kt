            ...
            for (record in parser) {
                ...
                when (type) {
                    STOCKS_TYPE_DIVIDEND -> {
                        ...
                        // Keep an explicit zero row when whtRate == 0: report preparation requires
                        // every dividend to have a matching tax record so missing parser data cannot
                        // be mistaken for a genuine zero-withholding case.
                        val taxAmount = if (wht == 0.0) 0.0 else -wht
                        taxes.add(TaxRecord(date, taxAmount, currency, symbol = ticker, broker = BROKER_NAME))
                    }
