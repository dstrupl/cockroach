...
class DegiroAccountStatementParserTest {
    ...
    @ParameterizedTest
    @EnumSource(Format::class)
    fun ignoresAdrFeesAndUnrelatedRowsAndEmitsExplicitZeroTax(format: Format, @TempDir tempDir: File) {
        val file = File(tempDir, "degiro.${format.extension}")
        createWorkbook(format, file, listOf(
        ...
        assertThat(result.taxRecords).containsExactly(
            TaxRecord(LocalDate(2024, 5, 10), 0.0, Currency.USD, symbol = "APPLE INC", broker = "Degiro")
        )
    }
