...
class DividentReportPreparationTest {
    ...
    @Test
    fun explicitZeroTaxRecordAllowsZeroWithholdingDividend() {
        val date = LocalDate(2025, 6, 15)
        val report = DividentReportPreparation.generateDividendReport(
            listOf(DividendRecord(date, 50.0, Currency.USD, "ZERO", "TestBroker", "US")),
            listOf(TaxRecord(date, 0.0, Currency.USD, "ZERO", "TestBroker")),
            emptyList(),
            year2025,
            fixedRate,
        )

        val usd = report.sections.single()
        assertThat(usd.totalBrutto).isEqualTo(50.0)
        assertThat(usd.totalTax).isEqualTo(0.0)
    }

    @Test
    fun czechSourceDividendPaidInForeignCurrencyFailsLoudly() {
        val date = LocalDate(2025, 6, 15)

        assertThatThrownBy {
            DividentReportPreparation.generateDividendReport(
                listOf(DividendRecord(date, 50.0, Currency.USD, "CEZ", "TestBroker", "CZ")),
                listOf(TaxRecord(date, -7.5, Currency.USD, "CEZ", "TestBroker")),
                emptyList(),
                year2025,
                fixedRate,
            )
            ...
            .isInstanceOf(IllegalStateException::class.java)
            .hasMessageContaining("Czech-source dividend")
            .hasMessageContaining("paid in USD")
    }
