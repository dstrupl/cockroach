...
[VÚB](https://www.vub.sk/), and [Interactive Brokers](https://www.interactivebrokers.com/)
services in the
...
more VÚB CZK account-statement PDFs, and optionally an Interactive Brokers Transaction History CSV,
then creates a summary of your sales, purchases, dividends and interest for the tax year.
...
1. Open **Transaction & History → Transaction History**.
2. Select **Custom** and choose the tax-year start and end dates.
3. Click the **CSV** icon to download the export.
4. Save the CSV file into `input/ib/dividends/`.

![](media/ib_transactions.png)

Notes:
- Initial support covers US-source dividends reported in a USD-base-currency account.
- `Dividend` and `Foreign Tax Withholding` rows are paired by date and symbol.
- Non-US-looking ticker suffixes and non-USD base currencies fail loudly rather than being
  silently routed to the wrong Czech tax-return section.
...
- Only the **Dividends** sheet is read. The parser converts each row into a gross
  `DividendRecord` (= net + WHT) and a matching `TaxRecord` (= -WHT, including explicit zero).
...
├── ib/                             # Interactive Brokers data directory
│   └── dividends/   *.csv          # single Transaction History export
...
ib: ./input/ib                        # optional Interactive Brokers directory
```
...
At least one of `schwab`, `etrade`, `degiro`, `revolut.stocks`, `revolut.savings`, `etoro`, `vub`, `ib` must be present.

## Positional arguments (legacy, Schwab + E-Trade + Interactive Brokers only)
...
-   it gets 3 required command line arguments - path to Schwab JSON export, year, and
    output dir. An optional 4th argument specifies the E-Trade data directory and an
    optional 5th argument specifies the Interactive Brokers data directory.
...
With E-Trade and Interactive Brokers data:

java -jar target/cockroach-0.3-SNAPSHOT.jar input/schwab-export.json 2025 ./output input/etrade input/ib

With YAML config (Schwab and/or E-Trade and/or Degiro and/or Revolut and/or eToro and/or VÚB and/or Interactive Brokers):
